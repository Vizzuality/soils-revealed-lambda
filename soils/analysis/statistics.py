...
from soils.analysis.utils import rasterize, sort_dict
...
class SoilStatistics:
    ...
    def filter_dataset(self):
        ...
        """
        polygon = Polygon(self.geometry['features'][0]['geometry']['coordinates'][0])
        xmin, ymin, xmax, ymax = polygon.bounds
        self.ds = self.ds.sel(lon=slice(xmin, xmax), lat=slice(ymax, ymin))
        shapes = zip([polygon], range(1))
        da_mask = rasterize(shapes, self.ds.coords, longitude='lon', latitude='lat').rename('mask')
        self.ds['mask'] = da_mask

    def get_area(self) -> float:
        """
        Calculate the area of the provided geometry.
    ...
    def compute_statistics(self):
        ...
        if self.dataset == 'historic':
            ...
            start_date = np.datetime64(datetime.strptime(f'{self.years[0]}-12-31', "%Y-%m-%d"))
            end_date = np.datetime64(datetime.strptime(f'{self.years[1]}-12-31', "%Y-%m-%d"))

        ds_index = self.ds.where(self.ds['mask'].isin(0.0)).sel(depth=self.depth, time=slice(start_date, end_date))
    ...
    def get_statistics(self) -> dict:
        ...
        data.update({'area_ha': self.get_area()})
...
class LandCoverStatistics:
    ...
    def filter_dataset(self):
        ...
        shapes = zip([polygon], range(1))
        da_mask = rasterize(shapes, self.ds.coords, longitude='x', latitude='y').rename('mask')
        self.ds['mask'] = da_mask
        # Filter by mask
    ...
    def compute_recent_statistics(self):
        ...
        """
        # Filter dataset
        df = pd.concat([self.ds.isel(time=0).to_dataframe().reset_index().drop(
            columns=['x', 'y', 'time']).rename(columns={'stocks': 'stocks_2000', 'land-cover': 'land_cover_2000'}),
                        self.ds.isel(time=1).to_dataframe().reset_index().drop(
                            columns=['x', 'y', 'time', 'mask']).rename(
                            columns={'stocks': 'stocks_2018', 'land-cover': 'land_cover_2018'})], axis=1)
        ...
        df = df[df['stocks_change'].notnull() & (df['stocks_change'] != 0.)]
        # Add category names
        ...
        df = df.assign(land_cover_group_2000=df['land_cover_2000'].map(self.lc_params.child_parent()),
                       land_cover_group_2018=df['land_cover_2018'].map(self.lc_params.child_parent()))
        ...
        for parent_id in list(self.data['land_cover_groups'].keys()):
            ...
            land_cover_dict[parent_id] = {id: self.data['land_cover'][id] for id in child_ids}
